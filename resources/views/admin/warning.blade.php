...
@section('content')
    <div class="warning-container p-6 md:p-[30px] m-0 w-full box-border">
        <div class="warning-header mb-6">
            ...
            <p class="last-update text-sm text-slate-500 m-0">Pembaruan terakhir:
                {{ \Carbon\Carbon::parse(str_replace(' WIB', '', $checkedAt))->translatedFormat('d F Y, H:i') }} WIB</p>
        </div>
        ...
        @if($warning)
            ...
            <div
                ...
                <div
                    class="warning-card-header p-4 md:p-6 flex flex-col md:flex-row justify-between md:items-start gap-4 md:gap-8">
                    <div class="warning-title-wrapper flex gap-4 items-start flex-1">

                        @if($iconType === 'petir')
                            <svg class="shrink-0 w-6 h-7 md:w-8 md:h-8 mt-1 md:mt-1.5" viewBox="0 0 24 24" fill="none"
                        ...
                        <div class="warning-title-text w-full md:pr-6">
                            <p class="text-orange-600 font-bold text-xs md:text-sm m-0 mb-1 capitalize tracking-wide">
                                {{ strtolower($warning['event'] ?? 'Cuaca Ekstrem') }}
                            </p>

                            <h2 class="text-sm md:text-base font-bold text-slate-800 m-0 mb-1.5 md:mb-3">{{ $warning['title'] }}
                            </h2>
                            <p
                                class="warning-summary text-justify text-xs md:text-sm text-slate-600 m-0 mb-2.5 md:mb-5 leading-relaxed">
                                {{ $warning['description'] }}
                            </p>
                            <div
                                class="warning-time-badge inline-block bg-orange-600 text-white px-3 md:px-4 py-1 md:py-2 rounded-full text-[11px] md:text-xs font-semibold mb-2 md:mb-4">
                                Berlaku: {{ $warning['effective_wib'] }} - {{ $warning['expires_wib'] }}
                            </div>
                        </div>
                    </div>

                    <a href="{{ route('admin.warning.detail') }}"
        ...
        @else
            <div
                class="empty-state-card bg-emerald-50 border border-emerald-200 rounded-lg p-[40px_20px] md:p-[60px_40px] text-center text-emerald-800 w-full box-border">
                <div class="empty-state-icon flex justify-center mb-4">
                    <svg class="w-10 h-10 md:w-12 md:h-12" viewBox="0 0 24 24" fill="none" stroke="#10b981" stroke-width="2.5"
                ...
                <p class="empty-state-desc text-xs md:text-base m-0">Saat ini tidak ada peringatan dini cuaca ekstrem untuk
                    wilayah Jawa Timur.</p>
            </div>
        @endif
    </div>
@endsection