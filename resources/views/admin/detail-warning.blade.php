...
@section('content')
    <div class="warning-map-container pt-6 px-6 pb-10 md:pt-[30px] md:px-[40px] md:pb-[40px] m-0 w-full box-border flex flex-col h-screen">
        ...
        <div class="mb-2 flex items-center justify-between shrink-0">
            <div>
                <h1 class="page-title text-lg font-bold text-slate-800 m-0 mb-1">Peta Persebaran Cuaca Ekstrem</h1>
                <p class="last-update text-sm text-slate-500 m-0 mb-5">Pembaruan terakhir: {{ \Carbon\Carbon::parse(str_replace(' WIB', '', $checkedAt))->translatedFormat('d F Y, H:i') }} WIB</p>
            </div>
